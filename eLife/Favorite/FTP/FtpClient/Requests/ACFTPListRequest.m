    ...
    do {
        ...
        if (bytesConsumed > 0) {
            ...
            if (thisEntry != NULL) {
                ...
                //entryToAdd = [self _entryByReencodingNameInEntry:(NSDictionary *) thisEntry encoding:0x80000632];//gb2312
                entryToAdd = [self _entryByReencodingNameInEntry:(NSDictionary *) thisEntry encoding:FTP_ENCODING];//gb2312
...
-(void)didListItems {
	...
	if(self.delegate != nil && [(NSObject*)self.delegate respondsToSelector:@selector(request:didListEntries:)]) {
		...
		for(NSDictionary* entry in self.listEntries) {
			NSLog(@"FTPListRequest Entry: %@", [entry description]);
		}
	}
}
