...
//    NSString *escapeStr = [url.absoluteString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//	ftpStream = CFReadStreamCreateWithFTPURL(NULL, (CFURLRef)[NSURL URLWithString:escapeStr]);
    
    
//    NSString *escapeStr = [@"ftp://admin:admin@10.36.24.52/儿童房.panel.zip" stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//    url = [NSURL URLWithString: escapeStr];
    ...
    assert(self.networkStream != NULL);
    
	self.networkStream.delegate = self;
	[self.networkStream scheduleInRunLoop:runLoop forMode:NSDefaultRunLoopMode];
    
    NSLog(@"streamStatus %d",self.networkStream.streamStatus);
