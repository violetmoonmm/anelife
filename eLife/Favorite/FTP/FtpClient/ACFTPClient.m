...
-(void)requestDidCancel:(id)request {
    if(self.delegate && [(NSObject*)self.delegate respondsToSelector:@selector(client:requestDidCancel:)]) {
...
-(void)dealloc {
    ...
    NSArray *array = [NSArray arrayWithArray:requests];
    
    for (ACFTPListRequest *request in array)
    {
        [request cancel];
