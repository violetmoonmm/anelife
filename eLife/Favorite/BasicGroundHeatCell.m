...
@interface BasicGroundHeatCell () <DeviceCtrlBgdViewDelegate>
{
    UIImageView *iconView;
    UILabel *nameLbl;
    
    UIImageView *bgdView;
    GroundHeatView *deviceController;
}

@end

@implementation BasicGroundHeatCell
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        iconView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"GroundHeatOffline"]];
...
- (void)onClick
{
    if ([self.device.state online]) {//在线才可以点击进入控制界面
        ...
        GroundHeatView *ctrlView = [[GroundHeatView alloc] initWithNibName:nibName bundle:nil];
...
- (void)displayDeviceStatus
{
    ...
    if (!self.device.state.online) {
        ...
        [iconView setImage:[UIImage imageNamed:@"GroundHeatOffline"]];
    }
    else if (self.device.state.powerOn) {
        
        
        [iconView setImage:[UIImage imageNamed:@"GroundHeatOn"]];
        
    }
    else {
        
        [iconView setImage:[UIImage imageNamed:@"GroundHeatOff"]];
    }
}
