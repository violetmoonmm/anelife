...
http_trans_connect(http_trans_conn *a_conn)
{
    ...
    /* set up the socket */
#ifdef WIN32 //windows
    if ((a_conn->sock = socket(AF_INET, SOCK_STREAM, 0)) == SOCKET_ERROR )
#else //linux
    ...
    {
        //	int timeout = 1;
        //	setsockopt(a_conn->sock, SOL_SOCKET, SO_SNDTIMEO, (char*)&timeout, sizeof(timeout));
        //	setsockopt(a_conn->sock, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(timeout));
    }
...
#else //linux
        int  iBlock = fcntl(a_conn->sock, F_GETFL, 0);
    if ( -1 != iBlock )
    {
        iBlock |= O_NONBLOCK;
        int iRet = fcntl(a_conn->sock, F_SETFL, iBlock);
        if ( -1 == iRet )
        {
            return -1;
        }
...
#endif
    {
        ...
        if (select(a_conn->sock+1, NULL, &set, NULL, &tm) > 0)
        {
            getsockopt(a_conn->sock, SOL_SOCKET, SO_ERROR, (char*)&error, &len);
            if (error)
            {
                a_conn->error_type = http_trans_err_type_errno;
                a_conn->error = error;
    ...
    iBlock = fcntl(a_conn->sock, F_GETFL, 0);
    if ( -1 != iBlock )
    {
        // iBlock ^= O_NONBLOCK;
        iBlock &= ~O_NONBLOCK;
...
http_trans_read_into_buf(http_trans_conn *a_conn)
{
    ...
    if ((a_conn->last_read = l_read = recv(a_conn->sock,
                                           ...
                                           l_bytes_to_read,0)) == SOCKET_ERROR )
    {
    ...
    http_trans_write_buf(http_trans_conn *a_conn)
    {
        ...
        if ((a_conn->last_read = l_written = send(a_conn->sock,
                                                  ...
                                                  a_conn->io_buf_io_left,0)) == SOCKET_ERROR )
        {
            if (WSAGetLastError() == WSAEWOULDBLOCK )
#else //linux
