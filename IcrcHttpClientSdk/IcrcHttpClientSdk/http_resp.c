...
int
http_resp_read_headers2(http_resp *a_resp, char *io_buf, int io_buf_use, char *l_start_body)
{
  int            l_done = 0;
  ...
  /* check to make sure that there's enough that came back */
	if( io_buf_use < 14 )
	{
		// "The http response line was too short."
		return -1;
	}
	l_ptr = io_buf;
  ...
  if (strncmp(l_ptr, "HTTP", 4) == 0)
	...
	else if(strncmp(l_ptr, "POST", 4) == 0)
	{
		// 服务器主动发送消息
		l_cur_ptr= l_ptr;
		goto L_case_post;
	...
	{
		// "The http response line did not begin with \"HTTP\""
		return -1;
	}
  ...
  if ((isdigit(l_ptr[5]) == 0) || /* http ver */
      ...
      (l_ptr[12] != ' '))          /* space */
	{
      // "Error parsing http response line"
      return -1;
	}
  /* convert char into int */
  a_resp->http_ver = (l_ptr[5] - 0x30);
  a_resp->http_ver += ((l_ptr[7] - 0x30) / 10.0f);
  ...
  while (l_done == 0)
	{
	 ...
	 headers as determined above by the _patt() call */
      ...
      /* reading the header name */
		if (l_state == reading_header)
		{
			...
			if (l_header_len == 0)
			{
				...
				/* bomb if it's the first header.  That's not valid */
					...
					{
						// "The first http response header began with whitespace"
						return -1;
					}
					l_cur_ptr++;
			...
			if (*l_cur_ptr == ':')
			{
				...
				if (l_header_len == 0)
				{
					// "An http response header was zero length"
					return -1;
				}
			...
			{
				// "Failed to find seperator in http response headers"
				return -1;
			}
			else
			{
		...
		{
			// "Unknown state while reading http response headers";
			return -1;
		}
  }

	l_content_length = http_hdr_get_value(a_resp->headers, http_hdr_Content_Length);
	/* if there's a content length, do your stuff */
	if( l_content_length )
	{
		if (string_is_number(l_content_length) == 0)
		{
			// "Content length in http response was not a number";
			return -1;
		}
...
http_resp_reading_header_jump:
		...
		if (l_start_body != NULL)
			l_done = 1;
		if ((l_done == 0) && (l_rv == HTTP_TRANS_NOT_DONE))
...
int
http_resp_read_body2(http_resp *a_resp,
		    http_req *a_req,
		    http_trans_conn *a_conn)
{
  int      l_return = 0;
...
//		l_return = read_body_content_length(a_resp, a_req, a_conn);

		{
			...
			l_left_to_read = l_len - a_conn->io_buf_alloc;
...
read_chunk(http_trans_conn *a_conn)
{
  ...
  if (a_conn->chunk_len == 0)
	{
		/* check to make sure that the pattern is in the buffer */
		do {
			if ((l_end_chunk_hdr =
