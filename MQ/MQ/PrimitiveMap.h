...
//namespace util{
     ...
     */
class  PrimitiveMap //: public std::map/*decaf::util::StlMap*/<std::string, PrimitiveValueNode>
{
    private:

       // PrimitiveValueConverter converter;

        std::map<std::string, PrimitiveValueNode> valueMap;
...
public:
		...
		const PrimitiveValueNode& get( const std::string& key ) const;
		//{

		//	typename std::map<std::string,PrimitiveValueNode>::const_iterator iter;
		...
		* {@inheritDoc}
		*/
		void put( const std::string& key, const PrimitiveValueNode& value );
		//{
		//	valueMap[key] = value;
		//}
    };
